...
console.log("[AP Classroom Assignments Modify (V2.1 LITE)]: Page loaded and script loaded, waiting for element.");
let check = true;
modifyCarousel = setInterval(()=>{
    if (document.getElementsByClassName("carousel-parent-container")[0] != undefined){
        if (check == true){
            console.log("[AP Classroom Assignments Modify (V2.1 LITE)]: Carousel found! Adding scroll bar and removing buttons.");
        ...
        if (check == false) console.log("[AP Classroom Assignments Modify (V2.1 LITE)]: Carousel disappeared! Waiting for element.");
        check = true;
    }
}, 1000);
